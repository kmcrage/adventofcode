...
for line in data.split('\n'):
    if line == '':
        ...
        if ('hgt' not in passport or not re.match('^[0-9]+$', passport['hgt'][:-2]) or
                ...
                (passport['hgt'][-2:] == 'in' and int(passport['hgt'][:-2]) > 76)):
            passport.pop('hgt', None)
        if 'hcl' not in passport or not re.match('^\#[0-9a-f]{6}$', passport['hcl']):
            passport.pop('hcl', None)
        if 'ecl' not in passport or passport['ecl'] not in ['amb', 'blu', 'brn', 'gry', 'grn', 'hzl', 'oth']:
            passport.pop('ecl', None)
